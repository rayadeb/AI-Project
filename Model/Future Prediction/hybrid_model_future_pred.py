...
def train_and_evaluate_2025_only(base_path, output_dir="Results"):
    ...
    # manually create playoff_df for 2024–25
    playoff_df_2025 = pd.DataFrame({
        'Team': playoff_teams_2025,
        'Season': ['2024-25'] * len(playoff_teams_2025),
        'Playoff_Rank': [0] * len(playoff_teams_2025)  # dummy value, not used in prediction
    })

    # preprocess only 2024–25 data
    X_player, X_team, y_dummy, meta = preprocess_data(player_df, playoff_df_2025)
    ...
    # train model
    train_loader = DataLoader(season_dataset, batch_size=8, shuffle=True)
    model.train()
